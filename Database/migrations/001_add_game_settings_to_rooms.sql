...
-- Date: 2025-12-29
-- Description: Add mode, max_players, round_time, wager_mode columns

-- Add columns with defaults
ALTER TABLE rooms 
  ADD COLUMN IF NOT EXISTS mode VARCHAR(20) DEFAULT 'scoring',
  ADD COLUMN IF NOT EXISTS max_players INT DEFAULT 5,
  ADD COLUMN IF NOT EXISTS round_time VARCHAR(20) DEFAULT 'normal',
...
WHERE table_name = 'rooms' 
  AND column_name IN ('mode', 'max_players', 'round_time', 'wager_mode')
