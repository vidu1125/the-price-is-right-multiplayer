...
static void* question_timer_thread(void *arg) {
    ...
    if (should_advance) {
        ...
        extern void advance_to_next_question(MessageHeader *req);
...
static int get_correct_index(int q_idx) {
    ...
    cJSON *data = cJSON_Parse(mq->json_data);
    if (!data) return -1;
    
    int correct = -1;
    
    // 1. Check "correct_index" field (number)
    ...
    // 2. Check "correct_answer" field - can be NUMBER or STRING
    else {
        cJSON *cans = cJSON_GetObjectItem(data, "correct_answer");
        if (cans) {
            if (cJSON_IsNumber(cans)) {
                // correct_answer is a NUMBER (index directly)
...
static void eliminate_player(MatchPlayerState *mp, const char *reason) {
    ...
    if (mp->match_player_id > 0) {
        ...
        cJSON_AddNumberToObject(event_payload, "round_no", g_r1.round_index + 1);
...
static void handle_submit_answer(int fd, MessageHeader *req, const char *payload) {
    ...
    // ⭐ SAVE ANSWER TO DATABASE (inline, no wrapper function)
    if (mp->match_player_id > 0 && round->questions[q_idx].question_id > 0) {
        // Build payload directly
        cJSON *payload = cJSON_CreateObject();
        cJSON_AddNumberToObject(payload, "question_id", round->questions[q_idx].question_id);
        cJSON_AddNumberToObject(payload, "player_id", mp->match_player_id);
        cJSON_AddNumberToObject(payload, "score_delta", delta);
        cJSON_AddNumberToObject(payload, "action_idx", 1);  // default action_idx = 1
        ...
        cJSON *result = NULL;
        db_error_t db_err = db_post("match_answer", payload, &result);
    ...
    cJSON_AddNumberToObject(result, "score_delta", delta);
