...
static char* build_end_game_json(EndGameResult *result) {
    ...
    cJSON_AddNumberToObject(obj, "room_id", result->room_id);
    cJSON_AddStringToObject(obj, "mode", result->mode == MODE_SCORING ? "scoring" : "elimination");
...
void broadcast_end_game_result(EndGameResult *result, MessageHeader *req) {
    ...
    if (match) {
        for (int i = 0; i < match->player_count; i++) {
            ...
            if (fd > 0) {
                forward_response(fd, req, NTF_GAME_END, json, (uint32_t)strlen(json));
            }
        }
    }
...
bool trigger_end_game(uint32_t match_id, int32_t bonus_winner_id) {
    ...
    if (match) {
        ...
        // =====================================================================
        // RETURN ALL PLAYERS TO ROOM
        // =====================================================================
        printf("[EndGame] Returning players to room %u...\n", match->room_id);
        
        // Reset the room state for a new game
        room_reset_for_new_game(match->room_id);
        
        // Change all players (survivors, eliminated, forfeited) back to LOBBY state
        // but KEEP them in the room
        for (int i = 0; i < match->player_count; i++) {
            MatchPlayerState *mp = &match->players[i];
            
            UserSession *session = session_get_by_account(mp->account_id);
            if (session) {
                session_mark_lobby(session);
                printf("[EndGame] Player %d returned to LOBBY state (staying in room %u)\n", 
                       mp->account_id, match->room_id);
            }
        }
    }
    
    // Cleanup match from memory
    match_destroy(match_id);
...
void handle_end_game_back_to_lobby(uint32_t match_id, int32_t account_id) {
    ...
    // REMOVE FROM ROOM: Ensure player is removed from room manager so they can join other rooms
    room_remove_member_all(session->socket_fd);
    printf("[EndGame] Removed player %d from all room records\n", account_id);
